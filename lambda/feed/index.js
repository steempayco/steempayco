...
exports.handler = async (event) => {
    ...
    var params = [
        {
            Item: {...result, priceFeedId: 'latest-price-feed', timestamp: Date.now()},
            TableName: 'priceFeed'
        },
        {
            Item: {...result, priceFeedId: feedId, timestamp: Date.now()},
            TableName: 'priceFeed'
        }
    ];
    
    params.map(param => docClient.put(param, function(err, data) {
        if (err) {
            console.log(err);
        } else {
            console.log(data);
