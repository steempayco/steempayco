...
let getSymbolFromCurrencyLocal = currencyCode => {
    if (currencyCode == "SBD") {
        return "SBD";
    } else {
...
let Utils = {
    ...
    getCurrencies: () => {
        return sortedCurrency.map(item => {
            return { ...item, symbol: getSymbolFromCurrencyLocal(item.code) };
        });
    },
    getCurrencySymbol: currencyCode => {
        return getSymbolFromCurrencyLocal(currencyCode);
    },
    ...
    currencyFormat: (value, code) => {
        let symbol = getSymbolFromCurrencyLocal(code);
        if (symbol.length > 1) {
