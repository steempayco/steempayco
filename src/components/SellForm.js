...
class SellForm extends Component {
    constructor(props) {
        super(props);
        let store =
            props.config.stores.length > 0 ? props.config.stores[0] : false;
    ...
    onStoreChange = (event, data) => {
        let store = this.props.config.stores.find(
            store => store.name === data.value
        );
    ...
    getUniqueItemList = () => {
        ...
        itemList.forEach(item => {
            let uniqItem = uniqItems.find(
                uniqItem => uniqItem.name === item.name
            );
            if (!uniqItem) {
        ...
        return (
            <div>
                ...
                <div style={{ textAlign: "right" }}>
                    <Label floated="right" color="orange" size="big" tag as="a">
                        {Utils.currencyFormat(totalAmount, currency)}
                    </Label>
                </div>
            </div>
        );
    ...
    getReceiverDetail = () => {
        ...
        let receiverInfo = [...config.users, ...config.exchanges].find(
    ...
    renderAllCase() {
        ...
        if (this.props.config.stores.length === 0) {
            return (
                <Message
                    ...
                    icon="help"
                    size="large"
                    header="You have no store"
                    content={
                        <span>
                            Add your store at <Link to="/setting">Setting</Link>{" "}
                            and try again.
                        </span>
                    }
                />
        ...
        let stores = this.props.config.stores.map((store, index) => {
            return { text: store.name, value: store.name, key: index };
        });
        ...
        return (
            <div>
                ...
                <Button
                    ...
                    fluid
                    onClick={() => this.createPayment(this.onPaymentCreated)}
                >
                    Sell!
                </Button>
                {this.state.fetching && (
